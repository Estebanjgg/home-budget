...
// La interfaz ahora incluye las propiedades que faltaban en `dashboardMetrics`.
interface FinancialHealthIndicatorsProps {
  dashboardMetrics: {
    ...
    savingsRate: number
    expenseRatio: number
    budgetCount: number // <--- Propiedad agregada
    totalTithe: number  // <--- Propiedad agregada
  }
  groceryMetrics: {
    totalSpent: number
...
function FinancialHealthIndicators({ dashboardMetrics, groceryMetrics, formatCurrency }: FinancialHealthIndicatorsProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100">
        <div className="text-center mb-8">
          ...
          <div className="text-center">
            <h4 className={`text-2xl font-bold mb-2 ${health.color === 'green' ? 'text-green-600' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100">
          ...
          <div className="relative mb-4">
            <div className="w-full bg-gray-200 rounded-full h-6">
              <div
                className={`h-6 rounded-full transition-all duration-1000 flex items-center justify-end pr-2 ${dashboardMetrics.expenseRatio <= 70 ? 'bg-gradient-to-r from-green-400 to-green-500' :
          ...
          <div className={`rounded-xl p-4 ${dashboardMetrics.expenseRatio <= 70 ? 'bg-green-50' :
              dashboardMetrics.expenseRatio <= 85 ? 'bg-yellow-50' : 'bg-red-50'
            }`}>
            <p className={`text-sm font-medium mb-2 ${dashboardMetrics.expenseRatio <= 70 ? 'text-green-800' :
                dashboardMetrics.expenseRatio <= 85 ? 'text-yellow-800' : 'text-red-800'
              }`}>
              💸 Gastos: {formatCurrency(dashboardMetrics.totalExpenses)}
            </p>
            <p className={`text-xs ${dashboardMetrics.expenseRatio <= 70 ? 'text-green-600' :
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100">
          ...
          <div className="relative mb-4">
            <div className="w-full bg-gray-200 rounded-full h-6">
              <div
                className={`h-6 rounded-full transition-all duration-1000 flex items-center justify-end pr-2 ${groceryMetrics.percentageOfIncome <= 15 ? 'bg-gradient-to-r from-green-400 to-green-500' :
          ...
          <div className={`rounded-xl p-4 ${groceryMetrics.percentageOfIncome <= 15 ? 'bg-green-50' :
              groceryMetrics.percentageOfIncome <= 25 ? 'bg-yellow-50' : 'bg-red-50'
            }`}>
            <p className={`text-sm font-medium mb-2 ${groceryMetrics.percentageOfIncome <= 15 ? 'text-green-800' :
                groceryMetrics.percentageOfIncome <= 25 ? 'text-yellow-800' : 'text-red-800'
              }`}>
              🛍️ Supermercado: {formatCurrency(groceryMetrics.totalSpent)}
            </p>
            <p className={`text-xs ${groceryMetrics.percentageOfIncome <= 15 ? 'text-green-600' :
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100">
          ...
          <div className="relative mb-4">
            <div className="w-full bg-gray-200 rounded-full h-6">
              <div
                className={`h-6 rounded-full transition-all duration-1000 flex items-center justify-end pr-2 ${groceryMetrics.efficiency >= 20 ? 'bg-gradient-to-r from-green-400 to-green-500' :
          ...
          <div className={`rounded-xl p-4 ${groceryMetrics.efficiency >= 20 ? 'bg-green-50' :
              groceryMetrics.efficiency >= 10 ? 'bg-yellow-50' : 'bg-red-50'
            }`}>
            <p className={`text-sm font-medium mb-2 ${groceryMetrics.efficiency >= 20 ? 'text-green-800' :
                groceryMetrics.efficiency >= 10 ? 'text-yellow-800' : 'text-red-800'
              }`}>
              💡 Eficiencia: {groceryMetrics.efficiency.toFixed(1)}%
            </p>
            <p className={`text-xs ${groceryMetrics.efficiency >= 20 ? 'text-green-600' :
